...
- **チャンク保管庫**: 過負荷チャンクから回収したアイテムを通常のゴミ箱と分離して保存し、一覧表示、閲覧権限付与が可能。
...
- **dustbin**: ゴミ箱を開く。
    - **引数リスト**:
        1. `[<ページ数>]`
- **chunkvault**: チャンク過負荷時に回収したアイテムを管理する。通常のゴミ箱とは別に保存されます。
    - **引数リスト**:
        1. `list [<ページ>]`: 保管庫 ID を一覧表示する。
        2. `open <ID> [<ページ>]`: 指定した保管庫を開く。
        3. `grant <ID> <プレイヤー>`: 指定した保管庫の閲覧権限を付与する。
- **sweep**: 手動で掃除をトリガーする。
    - **引数リスト**:
        1. `[<範囲>]`
        2. `[<ディメンション>]`
