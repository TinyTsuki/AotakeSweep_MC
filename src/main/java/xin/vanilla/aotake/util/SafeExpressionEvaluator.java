...
public class SafeExpressionEvaluator {
    ...
    private static boolean toBoolean(Object o) {
        return switch (o) {
            ...
            case String ignored -> StringUtils.stringToBoolean(o.toString());
            default -> true;
        };
    }
            ...
            switch (c) {
                ...
                default:
                    if (Character.isDigit(c)) {
                        ...
                        return switch (lid) {
                            case "true" -> new Token(TokenType.TRUE, id);
                            case "false" -> new Token(TokenType.FALSE, id);
                            case "null" -> new Token(TokenType.NULL, id);
    ...
    private record ClassAccessNode(VarNode base) implements Node {
        public Object evaluate(Map<String, Object> vars) {
    ...
    private record PropertyAccessNode(Node base, String prop) implements Node {
        public Object evaluate(Map<String, Object> vars) {
    ...
    private record FuncNode(String name, List<Node> args) implements Node {
        ...
        public Object evaluate(Map<String, Object> vars) {
            ...
            for (Node n : args) {
                ...
                switch (v) {
                    case null -> evalArgs.add(0.0);
                    case Number number -> evalArgs.add(number.doubleValue());
                    case String s when isNumericString(s) -> evalArgs.add(Double.parseDouble(s));
                    default -> throw new RuntimeException("Function " + name + " requires numeric args, got " + v);
                }
            }
            switch (name) {
                // 基本运算
                ...
                case "pow":
                    ...
                    return Math.pow(evalArgs.getFirst(), evalArgs.get(1));
                case "abs":
                ...
                case "toDegrees":
                    ...
                    return Math.toDegrees(evalArgs.getFirst());
                case "random": {
                    if (evalArgs.isEmpty()) {
                        return Math.random();
                    } else if (evalArgs.size() == 2) {
                        double min = evalArgs.getFirst(), max = evalArgs.get(1);
                        if (min > max) {
                            double t = min;
                            min = max;
                            max = t;
