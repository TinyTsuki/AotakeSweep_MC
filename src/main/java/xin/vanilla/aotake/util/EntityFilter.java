                ...
                case ACCESSOR_KEY:
                    ...
                    DataParameter<?> accessor = accessorCache.computeIfAbsent(accessorCacheKey, k -> {
                        try {
                            ...
                            if (split.length == 1) {
                                return (DataParameter<?>) FieldUtils.getPrivateFieldValue(FieldUtils.getClass(entity), entity, split[0], true);
                            }
                            Class<?> decl = FieldUtils.getClass(split[0]);
                            if (decl == null || !decl.isInstance(entity)) {
                                return null;
                            }
                            return (DataParameter<?>) FieldUtils.getPrivateFieldValue(decl, entity, split[1]);
                        } catch (Throwable ignored) {
                            return null;
                        }
                    });
