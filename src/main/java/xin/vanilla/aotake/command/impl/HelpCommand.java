...
public class HelpCommand {
    public static LiteralArgumentBuilder<CommandSourceStack> help() {
        Command<CommandSourceStack> helpCommand = context -> {
            ...
            Component helpInfo;
            if (page > 0) {
                int helpNum = ServerConfig.get().helpInfoNumPerPage();
                int pages = (int) Math.ceil((double) AotakeCommand.HELP_MESSAGE.size() / helpNum);
                helpInfo = Component.literal(StringUtils.format(helpNum + "\n", page, pages));
                for (int i = 0; (page - 1) * helpNum + i < AotakeCommand.HELP_MESSAGE.size() && i < helpNum; i++) {
                    KeyValue<String, EnumCommandType> keyValue = AotakeCommand.HELP_MESSAGE.get((page - 1) * helpNum + i);
        ...
        SuggestionProvider<CommandSourceStack> helpSuggestions = (context, builder) -> {
            ...
            int totalPages = (int) Math.ceil((double) AotakeCommand.HELP_MESSAGE.size() / ServerConfig.get().helpInfoNumPerPage());
